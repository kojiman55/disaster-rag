...
// src/weatherUpdate.ts
var weatherUpdate_exports = {};
__export(weatherUpdate_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(weatherUpdate_exports);
...
// src/weatherUpdate.ts
var FEED_URL = "https://www.data.jma.go.jp/developer/xml/feed/extra.xml";
async function fetchWarningsFromFeed() {
  ...
  const warnings = [];
  const warningEntries = entryList.filter((e) => e.title && (e.title.includes("\u8B66\u5831") || e.title.includes("\u6CE8\u610F\u5831"))).slice(0, 20);
  for (const entry of warningEntries) {
    const title = entry.title ?? "";
    const updated = entry.updated ?? (/* @__PURE__ */ new Date()).toISOString();
